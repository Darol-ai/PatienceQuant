"""消融实验专用（docs/adr/0012）：分散化候选池 + 官方14项特征（不加新
特征），隔离"只换股票池"这一个变量，和 run_diversified_per_year_backtest.py
（分散化+16项特征）、run_v3_per_year_backtest.py（官方10支+14项特征）
三者对比。

用法：
    conda activate stock
    cd PatienceQuant/backend
    python scripts/run_diversified_base_per_year_backtest.py
"""
...
def run_one_year(history: pd.DataFrame, year: int) -> dict:
    signal_source = WalkForwardSignalSource(
        DATA_DIR / "diversified_base_model_walkforward", DATA_DIR / "diversified_universe_history.parquet"
    )
    strategy = V3Strategy(
