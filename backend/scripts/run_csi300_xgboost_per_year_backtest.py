"""沪深300universe版本的XGBoost策略逐年回测验证——和LightGBM版本
(run_csi300_lightgbm_per_year_backtest.py)完全同一套引擎/universe/
Top-K规则，只换信号源，用于验证"第二个独立有效策略"。

用法：
    conda activate stock
    cd PatienceQuant/backend
    python scripts/run_csi300_xgboost_per_year_backtest.py
"""
...
MODEL_DIR = "csi300_xgboost_ensemble_model_walkforward"
TOP_K_RATIO = 0.1
...
def run_one_year(history: pd.DataFrame, year: int) -> dict:
    signal_source = XGBoostWalkForwardSignalSource(DATA_DIR / MODEL_DIR, DATA_DIR / HISTORY_FILE)
