"""沪深300universe版本的LightGBM策略逐年回测验证——不搬30支候选池
版本调过的动量兜底/流动性判断这些参数(那些是针对30支小候选池调的，
换了300支大universe直接照搬没有依据，属于没做过验证的假设)。这里从
最简单的版本开始：纯Top-K相对排序，Top 30支(约10%，对齐主流"从大
universe里选20-30支集中持仓"的常见做法)，月度调仓，不叠加引擎级
止损/波动率控制(ADR-0037已证明这层对30支候选池版本是净拖累，这里
延用同一个结论作为起点，而不是重新假设它对大universe也一样)。

用法：
    conda activate stock
    cd PatienceQuant/backend
    python scripts/run_csi300_lightgbm_per_year_backtest.py
"""
...
MODEL_DIR = "csi300_lightgbm_ensemble_model_walkforward"
TOP_K_RATIO = 0.1
...
def run_one_year(history: pd.DataFrame, year: int) -> dict:
    signal_source = EnsembleRegressionWalkForwardSignalSource(DATA_DIR / MODEL_DIR, DATA_DIR / HISTORY_FILE)
